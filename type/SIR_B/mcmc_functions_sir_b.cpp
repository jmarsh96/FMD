...
public:
  ...
  double t_max; // when does the outbreak end
           ...
           NumericVector parameters_)
{
  ...
  // Determine the initial infective since we do not model their transmission
  int initial_infective_;
  double min_time = *max_element(t_i.begin(), t_i.end());
  double t_max_ = *max_element(t_r.begin(), t_r.end());
  t_max = t_max_;
  for(auto i : ever_infected)
  {
    double cur_time = t_i[i];
    if(cur_time < min_time)
    {
      min_time = cur_time;
      initial_infective_ = i;
    }
  }
  initial_infective = initial_infective_;
...
void Data::CalculateTransmissionLikelihood(bool verbose)
{
  ...
  transmission_likelihood_ -= beta*double_sum;
  
  for(size_t i = 0; i < N; i++)
  {
    if(i != initial_infective)
    {
      int cur_source = source[i];
      if(cur_source == -1)
      {
        // background infection
        transmission_likelihood_ += log(beta0);
      }
      else if(cur_source >= 0)
      {
        double cur_dist = CalculateDistance(i,cur_source);
        transmission_likelihood_ += log(beta) - kappa*cur_dist;
      }
      
      // add contribution from avoiding background infection
      double inf_time = t_i[i];
      if(inf_time == -1) inf_time = t_max;
      transmission_likelihood_ -= beta0*inf_time;
  ...
  if(verbose)
  {
    std::cout << " After observation, likelihood = " << transmission_likelihood_ << std::endl;
  }
...
// [[Rcpp::export]]
void MCMC_SIR_B(List MCMC_options, 
                NumericVector t_i, 
                NumericVector t_r,
                NumericVector t_s,
                ...
                NumericVector y)
{
  ...
  for(int iter = 1; iter < max_iterations; iter++)
  {
    ...
    // Update beta0 by MH
    int beta0_flag = debug_flags["beta0"];
    if(beta0_flag==0)
    {
      
      double prop_var = proposal_variance["beta0"];
      nacc_beta0_prop++;
  ...
  double beta0_prob = (double)nacc_beta0/(double)nacc_beta0_prop;
