...
void Data::CalculateContribRevUpper(int imp_idx, bool verbose)
{
...
void Data::CalculateContribRevExterior(int imp_idx, bool verbose)
{
...
void Data::CalculateContribRevInterior(int imp_idx, bool verbose)
{
...
void Data::UpdateImputedNodes()
{
  ...
  for(auto imp_idx_cur : idx_to_impute_cur)
  {
    ...
    // Eventually find a way to partial update - will improve mixing
    // but not important right now
    //bool calculate_contribution = CalculateContribution(imp_idx_cur,
    //                                                   data_cur, (*this));
    ...
    if(calculate_contribution)
    {
      ...
      case 1:
        // Interior node
        data_cur.CalculateContribRevInterior(imp_idx_cur, verbose);
        break;
      case 2:
        // Exterior node
        data_cur.CalculateContribRevExterior(imp_idx_cur, verbose);
        break;
      case 3:
        // Upper node
        data_cur.CalculateContribRevUpper(imp_idx_cur, verbose);
        break;
      default:
                              ...
                              IntegerMatrix &gen_matrix_can, bool verbose)
{
  ...
  // don't bother partial updating for now
  ...
  if(same_config)
  {
    ...
    int loc_in_cur = data_cur.FindGeneticLoc(current_id, current_time,
                                             current_subtype);
    parent_distance = data_cur.gen_matrix(loc_in_cur, min_i);
    
    if(verbose)
    {
      std::cout << "  imp_idx_cur = " << imp_idx 
                << ", loc in cur = " << loc_in_cur
                << ", min_i = "
...
// [[Rcpp::export]]
void MCMC_SIR_JC(List MCMC_options,
                 int sequence_length,
                 NumericVector t_i, 
                 NumericVector t_r,
                 IntegerVector source,
