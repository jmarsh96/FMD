...
public:
  ...
  IntegerMatrix T_matrix;
  IntegerMatrix V_matrix;
  ...
  Data(int sequence_length_,
       ...
       IntegerMatrix T_matrix_,
       IntegerMatrix V_matrix_);
  ...
  void ImputeUpperNode(int imp_idx, Data data_cur, IntegerMatrix &T_matrix_can, 
                       IntegerMatrix &V_matrix_can, bool verbose = false);
  void ImputeExteriorNode(int imp_idx, Data data_cur, IntegerMatrix &T_matrix_can, 
                          IntegerMatrix &V_matrix_can, bool verbose = false);
  void ImputeInteriorNode(int imp_idx, Data data_cur, IntegerMatrix &T_matrix_can, 
                          IntegerMatrix &V_matrix_can, bool verbose = false);
  ...
  int ReturnSourceSequence2(const int sequence_loc, bool verbose = false);
  ...
  int CalculateTDistanceBetweenNodes(int node_i, int node_j);
  int CalculateVDistanceBetweenNodes(int node_i, int node_j);
...
void TriangleInequalityCheck(Data data)
{
  for(size_t j = 1; j < data.genetic_ids.size(); j++)
  {
    for(size_t i = 0; i < j; i++)
    {
      ...
      int dist_sum = 1;
      int dist_T = data.T_matrix(node_i,node_j);
      if(dist_sum > dist_T)
      {
...
// p_t, p_v and p_n are the probabilities of observing a transition, transversion and nothing
// where p_t + p_v + p_n = 1  (note p_v is twice the markov transition probability)
struct K80p {
  double p_t;
  double p_v;
  double p_n;
...
K80p CalculateMutationProbability(double transition_rate, double transversion_rate, double time_diff)
{
  K80p probs;
  
  probs.p_t = 0.25 + 0.25*exp(-4*transversion_rate*time_diff) - 
    0.5*exp(-2*(transition_rate + transversion_rate)*time_diff);
  probs.p_v = 2*(0.25 - 0.25*exp(-4*transversion_rate*time_diff));
  probs.p_n = 1 - probs.p_t - probs.p_v;
...
void Data::CalculateGeneticLikelihood(bool verbose)
{
  ...
  double transition_rate = parameters[6]; // mutation rates
  double transversion_rate = parameters[7]; 
  ...
  for(size_t i = 0; i < gen_source.size(); i++)
  {
    ...
    if(cur_source != -1)
    {
      ...
      int num_T = T_matrix(i, cur_source);
      int num_V = V_matrix(i, cur_source);
      
      double time_diff = sample_times[i] - sample_times[cur_source];
      K80p mut_prob = CalculateMutationProbability(transition_rate, transversion_rate, time_diff);
        
      double contribution = (sequence_length-num_T-num_V)*log(mut_prob.p_n) + 
        num_T*log(mut_prob.p_t) + num_V*log(mut_prob.p_v/2);
      genetic_likelihood_ += contribution;
      direct_contrib += contribution;
      if(verbose)
      {
        std::cout << "Calculate likelihood for sequence " << i
                  << " with source sequence " << cur_source << " with T_dist = "
                  << num_T << ", V_dist = " << num_V << ", time_diff = " << time_diff
                  << ", p_T = " << mut_prob.p_t << ", p_V = " << mut_prob.p_v
                  << " and contribution = " << contribution << std::endl;
      }
      if(num_T < 0 || num_V < 0) stop("Negative distances in the matrix");
    ...
    for(auto imp_node : imp_clust_originators)
    {
      ...
      for(size_t j = 1; j < obs_nodes.size(); j++)
      {
        for(size_t i = 0; i < j; i++)
        {
          ...
          int dist_T = T_matrix(node_i, node_j);
          int dist_V = V_matrix(node_i, node_j);
          int dist_sum_T = CalculateTDistanceBetweenNodes(node_i, node_j);
          int dist_sum_V = CalculateVDistanceBetweenNodes(node_i, node_j);
          int discrepancy_T = dist_sum_T - dist_T;
          int discrepancy_V = dist_sum_V - dist_V;
          
          if(discrepancy_T < 0 || discrepancy_V < 0)
          {
          ...
          if(verbose)
          {
            std::cout << "For nodes " << node_i << " and " << node_j
                      << ", dist_T = " << dist_T << ", dist_V = " << dist_V << ", dist_sum_T = "
                      << dist_sum_T << ", dist_sum_V = " << dist_sum_V << ", discrepancy_T = "
                      << discrepancy_T << ", discrepancy_V = " << discrepancy_V;
          }
          
          double discrepancy_prob = 0.9;
          double contribution_T = R::dgeom(discrepancy_T, discrepancy_prob, 1);
          double contribution_V = R::dgeom(discrepancy_V, discrepancy_prob, 1);
          
          genetic_likelihood_ += contribution_T + contribution_V;
          if(verbose)
          {
            std::cout << ", contribution_T = " << contribution_T
                      << ", contribution_V = " << contribution_V << std::endl;
          }
        }
      }
    }
  ...
  for(size_t i = 0; i < import_idx.size(); i++)
  {
    ...
    K80p mut_prob = CalculateMutationProbability(transition_rate, transversion_rate, time_diff);
    double mutation_prob = mut_prob.p_t + mut_prob.p_v;
...
void Data::WriteConfigurationToFile(std::ofstream &myfile)
{
  ...
  // Consider adding printing of T and V matrices
}
...
int Data::ReturnSourceSequence2(const int sequence_loc, bool verbose)
{
  ...
  while(current_target != -1)
  {
    ...
    for(auto cur_seq : target_sequences)
    {
      ...
      double seq_time = sample_times[cur_seq];
      int seq_subtype = subtype_numbers[cur_seq];
      if(seq_time < current_time &&
         seq_subtype == current_subtype &&
         seq_time > source_sequence_time &&
         cur_seq != sequence_loc)
      {
        source_sequence = cur_seq;
        source_sequence_time = seq_time;
      }
    }
    
    if(source_sequence != -1) return source_sequence;
    
    double eps = 0.0001;
    current_time = t_e[current_target] + eps;
...
{
  //verbose = false;
  if(sequence_loc == 164) verbose = true;
  if(sequence_loc == 98) verbose = true;
  if(sequence_loc == 113) verbose = true;
  int current_target = genetic_ids[sequence_loc];
  double current_time = sample_times[sequence_loc];
  int current_subtype = subtype_numbers[sequence_loc];
  if(verbose)
...
int Data::CalculateVDistanceBetweenNodes(int node_i, int node_j)
{
  bool verbose = false;
  ...
  if(same_pathway)
  {
    ...
    if(path_i.size() > path_j.size())
    {
      ...
      dist_sum += V_matrix(target, parent);
      while(parent != node_j)
      {
        target = parent;
  ...
  else
  {
    ...
    if(verbose)
    {
      std::cout << " target = " << target << ", parent = "
                << parent << ", dist = " << V_matrix(target, parent)
                << std::endl;
    }
    dist_sum += V_matrix(target, parent);
    while(parent != LCA)
...
int Data::CalculateTDistanceBetweenNodes(int node_i, int node_j)
{
  bool verbose = false;
  //if(node_i == 3 && node_j == 8) verbose = true;
  if(verbose)
  ...
  if(same_pathway)
  {
    ...
    if(path_i.size() > path_j.size())
    {
      ...
      int target = node_i;
      int parent = gen_source[node_i];
      dist_sum += T_matrix(target, parent);
      while(parent != node_j)
      {
        target = parent;
        parent = gen_source[target];
  ...
  else
  {
    ...
    {
      std::cout << " target = " << target << ", parent = "
                << parent << ", dist = " << T_matrix(target, parent)
                << std::endl;
    }
    dist_sum += T_matrix(target, parent);
    while(parent != LCA)
...
void Data::CalculateContribRevUpper(int imp_idx, Data data_can, bool verbose)
{
  ...
  for(auto ch_node : child_nodes)
  {
    ...
    double transition_rate = parameters[6]; 
    double transversion_rate = parameters[7]; 
    K80p mut_prob = CalculateMutationProbability(transition_rate, transversion_rate, time_diff);
    
    int draw_T = T_matrix(imp_idx, ch_node);
    int draw_V = V_matrix(imp_idx, ch_node);
    int draw_total = draw_T + draw_V;
    double contribution = R::dbinom(draw_total, sequence_length, mut_prob.p_t + mut_prob.p_v, 1) + 
      R::dbinom(draw_T, draw_total, mut_prob.p_t/(mut_prob.p_t + mut_prob.p_v), 1);
    ...
    if(verbose)
    {
      std::cout << "  Current child node = " << ch_node << " from ID = "
                ...
                << sample_times[ch_node] << ", draw_T = " << draw_T
                << ", draw_V = " << draw_V
                << ", contribution = " << contribution
                << std::endl;
    }
...
void Data::CalculateContribRevExterior(int imp_idx, Data data_can, bool verbose)
{
  ...
  int min_distance_T = sequence_length;
  int min_distance_V = sequence_length;
  ...
  for(size_t i = 0; i < direct_observed_child_nodes.size(); i++)
  {
    for(size_t j = 0; j < observed_child_nodes.size(); j++)
    {
      ...
      if(node_i != node_j)
      {
        int current_distance_T = T_matrix(node_i, node_j);
        int current_distance_V = V_matrix(node_i, node_j);

        // Check the T distances
        if(current_distance_T < min_distance_T)
        {
          min_distance_T = current_distance_T;
          min_i_T = node_i;
          min_j_T = node_j;
        }
        
        // Check the V distances
        if(current_distance_V < min_distance_V)
        {
          min_distance_V = current_distance_V;
          min_i_V = node_i;
          min_j_V = node_j;
        }
      }
  ...
  double upper_T = sample_times[min_i_T] - sample_times[imp_idx];
  double lower_T = sample_times[min_i_T] + sample_times[min_j_T] - 
    2*sample_times[imp_idx];
  double time_ratio_T = upper_T/lower_T;
  
  double upper_V = sample_times[min_i_V] - sample_times[imp_idx];
  double lower_V = sample_times[min_i_V] + sample_times[min_j_V] - 
    2*sample_times[imp_idx];
  double time_ratio_V = upper_V/lower_V;
  
  if(time_ratio_T < 0 || time_ratio_T > 1 || time_ratio_V < 0 || time_ratio_V > 1)
  {
    std::cout << "Time ratio = " << time_ratio_T << std::endl;
    stop("invalid time ratio");
  }
  
  
  
  int observed_distance_T = T_matrix(imp_idx, min_i_T);
  int observed_distance_V = V_matrix(imp_idx, min_i_V);
  double contribution = R::dbinom(observed_distance_T, min_distance_T, time_ratio_T, 1) + 
    R::dbinom(observed_distance_V, min_distance_V, time_ratio_V, 1);
  genetic_contribution += contribution;
  
  if(verbose)
  {
    std::cout << "  Different config, calculate contribution - "
              << " min_i = " << min_i_T << ", parent_distance = "
              << observed_distance_T << ", time_ratio = "
              << time_ratio_T << ", contribution = "
...
void Data::CalculateContribRevInterior(int imp_idx, Data data_can, bool verbose)
{
  ...
  double min_time_T = sample_times[min_child];
  double min_time_V = sample_times[min_child];
  int min_distance_T = T_matrix(parent_node, min_child);
  int min_distance_V = V_matrix(parent_node, min_child);
  ...
  for(size_t i = 1; i < observed_child_nodes.size(); i++)
  {
    ...
    int current_distance_T = T_matrix(parent_node, current_child);
    int current_distance_V = V_matrix(parent_node, current_child);
    
    if(current_distance_T <= min_distance_T)
    {
      min_distance_T = current_distance_T;
      double current_time = sample_times[current_child];
      min_time_T = std::min(min_time_T, current_time);
    }
    ...
    if(current_distance_V <= min_distance_V)
    {
      min_distance_V = current_distance_V;
      double current_time = sample_times[current_child];
      min_time_V = std::min(min_time_V, current_time);
    }
  }
  
  // Now perform the triangle inequality checks
  int triangle_min_T = ReturnMinDistanceTriangle(imp_idx, T_matrix);
  min_distance_T = std::min(min_distance_T, triangle_min_T);
  
  int triangle_min_V = ReturnMinDistanceTriangle(imp_idx, V_matrix);
  min_distance_V = std::min(min_distance_V, triangle_min_V);
  
  double upper = sample_times[imp_idx] - sample_times[parent_node];
  double lower_T = sample_times[min_child] - sample_times[parent_node];
  double time_ratio_T = upper/lower_T;
  double lower_V = sample_times[min_child] - sample_times[parent_node];
  double time_ratio_V = upper/lower_V;
  if((time_ratio_T < 0 || time_ratio_T > 1) || time_ratio_V < 0 || time_ratio_V > 1)
  {
    stop("Invalid time ratio");
  }
  
  int observed_distance_T = T_matrix(imp_idx, parent_node);
  int observed_distance_V = V_matrix(imp_idx, parent_node);
  double contribution = R::dbinom(observed_distance_T, min_distance_T, time_ratio_T, 1) + 
    R::dbinom(observed_distance_V, min_distance_V, time_ratio_V, 1);
  genetic_contribution += contribution;
  ...
  if(verbose)
  {
    std::cout << "  Different config, min_distance_T = "
              << min_distance_T << ", min_distance_V = "
              << min_distance_V << ", parent_distance_T = "
              << observed_distance_T << ", parent_distance_V = "
              << observed_distance_V << ", time_ratio_T = "
              << time_ratio_T << ", time_ratio_V = "
              << time_ratio_V << ", contribution = "
              << contribution
              << std::endl << std::endl;
  }
}

IntegerMatrix InitialiseCandidateMatrix_T(Data data_can)
{
  ...
  for(int i = 0; i < gen_matrix_can.nrow(); i++)
  {
    for(int j = 0; j < gen_matrix_can.ncol(); j++)
    {
      ...
      {
        gen_matrix_can(i,j) = data_can.T_matrix(i,j);
      }
      else
      {
...
IntegerMatrix InitialiseCandidateMatrix_V(Data data_can)
{
  size_t length_can = data_can.genetic_ids.size();
  IntegerMatrix gen_matrix_can(length_can, length_can);
  // Fill in entries with -1
  for(int i = 0; i < gen_matrix_can.nrow(); i++)
  {
    for(int j = 0; j < gen_matrix_can.ncol(); j++)
    {
      if(data_can.imputed_nodes[i] == 0 && data_can.imputed_nodes[j] == 0)
      {
        gen_matrix_can(i,j) = data_can.V_matrix(i,j);
      }
      else
      {
        gen_matrix_can(i,j) = -1;
...
void Data::UpdateImputedNodes()
{
  ...
  IntegerMatrix T_mat_can = InitialiseCandidateMatrix_T((*this));
  IntegerMatrix V_mat_can = InitialiseCandidateMatrix_V((*this));
  ...
  for(auto imp_idx_can : idx_to_impute_can)
  {
    ...
    case 1:
      ...
      ImputeInteriorNode(imp_idx_can, data_cur, T_mat_can, V_mat_can, verbose);
      break;
    case 2:
      // Impute exterior node
      ImputeExteriorNode(imp_idx_can, data_cur, T_mat_can, V_mat_can, verbose);
      break;
    case 3:
      // Impute upper node
      ImputeUpperNode(imp_idx_can, data_cur, T_mat_can, V_mat_can, verbose);
      break;
    default:
...
void Data::ImputeUpperNode(int imp_idx, Data data_cur, IntegerMatrix &T_matrix_can, 
                           IntegerMatrix &V_matrix_can, bool verbose)
{
  ...
  if(same_config)
  {
    ...
    for(auto ch_node : child_nodes)
    {
      ...
      int child_idx_cur = data_cur.FindGeneticLoc(child_id_can, samp_time_can,
                                                  subtype_can);
      int cur_dist_T = data_cur.T_matrix(imp_idx_cur, child_idx_cur);
      int cur_dist_V = data_cur.V_matrix(imp_idx_cur, child_idx_cur);
      T_matrix_can(imp_idx, ch_node) = cur_dist_T;
      T_matrix_can(ch_node, imp_idx) = cur_dist_T;
      
      V_matrix_can(imp_idx, ch_node) = cur_dist_V;
      V_matrix_can(ch_node, imp_idx) = cur_dist_V;
      ...
      if(verbose)
      {
        std::cout << "  Current child node = " << ch_node << " from ID = "
                  ...
                  << ", found in current data set - copy T distance = "
                  << cur_dist_T << " and V distance = " << cur_dist_V
                  << std::endl;
      }
    }
  }
  else
  {
    ...
    for(auto ch_node : child_nodes)
    {
      ...
      int total_mutations = R::rbinom(sequence_length, mut_prob.p_t + mut_prob.p_v);
      int total_transitions = R::rbinom(total_mutations, mut_prob.p_t/(mut_prob.p_t + mut_prob.p_v));
      int total_transversions = total_mutations - total_transitions;
      
      double contribution = R::dbinom(total_mutations, sequence_length, mut_prob.p_t + mut_prob.p_v, 1) + 
        R::dbinom(total_transitions, total_mutations, mut_prob.p_t/(mut_prob.p_t + mut_prob.p_v), 1);
      genetic_contribution -= contribution;
      if(verbose)
      {
        std::cout << "  Current child node = " << ch_node << " from ID = "
                  ...
                  << sample_times[ch_node] 
                  << ", p_T = " << mut_prob.p_t << ", p_V = " << mut_prob.p_v
                  << ", T = " << total_transitions << ", V = " << total_transversions
                  << ", contribution = " << contribution
                  << std::endl;
      }
      
      T_matrix_can(imp_idx, ch_node) = total_transitions;
      T_matrix_can(ch_node, imp_idx) = total_transitions;
      V_matrix_can(imp_idx, ch_node) = total_transversions;
      V_matrix_can(ch_node, imp_idx) = total_transversions;
...
void Data::ImputeExteriorNode(int imp_idx, Data data_cur, IntegerMatrix &T_matrix_can, 
                              IntegerMatrix &V_matrix_can, bool verbose)
{
  ...
  int parent_distance_T = -1;
  int parent_distance_V = -1;
  int min_i_T = -1;
  int min_i_V = -1;
  int min_distance_T = sequence_length;
  int min_distance_V = sequence_length;
  ...
  if(same_config)
  {
    ...
    parent_distance_T = data_cur.T_matrix(imp_idx_cur, min_i_T);
    parent_distance_V = data_cur.V_matrix(imp_idx_cur, min_i_T);
    ...
    if(verbose)
    {
      std::cout << "  imp_idx_can = " << imp_idx 
                ...
                << ", min_i = "
                << min_i_T << ", parent_distance = " << parent_distance_T
                << std::endl;
  ...
  else
  {
    ...
    parent_distance_T = R::rbinom(min_distance_T, time_ratio_T);
    parent_distance_V = R::rbinom(min_distance_V, time_ratio_V);
    
    // Calculate contribution
    double contribution = R::dbinom(parent_distance_T, min_distance_T, 
                                    time_ratio_T, 1) + R::dbinom(parent_distance_V, min_distance_V, 
                                    time_ratio_V, 1);
    genetic_contribution -= contribution;
  ...
  T_matrix_can(imp_idx, min_i_T) = parent_distance_T;
  T_matrix_can(min_i_T, imp_idx) = parent_distance_T;
  
  V_matrix_can(imp_idx, min_i_V) = parent_distance_V;
  V_matrix_can(min_i_V, imp_idx) = parent_distance_V;
  
  
  for(size_t i = 0; i < observed_child_nodes.size(); i++)
  {
    int current_child_node = observed_child_nodes[i];
    if(current_child_node != min_i_T)
    {
      T_matrix_can(imp_idx, current_child_node) = T_matrix_can(min_i_T, current_child_node) -
        T_matrix_can(imp_idx, min_i_T);
      T_matrix_can(current_child_node, imp_idx) = T_matrix_can(min_i_T, current_child_node) -
        T_matrix_can(imp_idx, min_i_T);
      ...
      if(print_extra)
      {
        std::cout << " current child node = " << current_child_node
                  << ", T_matrix_can(min_i, current_child_node) = "
                  << T_matrix_can(min_i_T, current_child_node)
                  << ", T_matrix_can(imp_idx, min_i) = "
                  << T_matrix_can(imp_idx, min_i_T)
                  << std::endl;
      }
    }
    
    if(current_child_node != min_i_V)
    {
      V_matrix_can(imp_idx, current_child_node) = V_matrix_can(min_i_V, current_child_node) -
        V_matrix_can(imp_idx, min_i_V);
      V_matrix_can(current_child_node, imp_idx) = V_matrix_can(min_i_V, current_child_node) -
        V_matrix_can(imp_idx, min_i_V);
    }
  }
  ...
  for(size_t jj = 0; jj < import_idx_can.size(); jj++)
  {
    ...
    {
      T_matrix_can(imp_idx, cur_imp_node) = T_matrix_can(min_i_T, cur_imp_node) -
        T_matrix_can(imp_idx, min_i_T);
      T_matrix_can(cur_imp_node, imp_idx) = T_matrix_can(min_i_T, cur_imp_node) -
        T_matrix_can(imp_idx, min_i_T);
      
      V_matrix_can(imp_idx, cur_imp_node) = V_matrix_can(min_i_V, cur_imp_node) -
        V_matrix_can(imp_idx, min_i_V);
      V_matrix_can(cur_imp_node, imp_idx) = V_matrix_can(min_i_V, cur_imp_node) -
        V_matrix_can(imp_idx, min_i_V);
    }
  }
}
...
void Data::ImputeInteriorNode(int imp_idx, Data data_cur, IntegerMatrix &T_matrix_can, 
                              IntegerMatrix &V_matrix_can, bool verbose)
{
  ...
  if(same_config)
  {
    ...
    int parent_cur = data_cur.gen_source[imp_idx_cur];
    parent_distance_T = data_cur.T_matrix(imp_idx_cur, parent_cur);
    parent_distance_V = data_cur.V_matrix(imp_idx_cur, parent_cur);
    if(verbose)
    {
      std::cout << "  Same config - parent_cur = " << parent_cur
                << ", parent_distance_T = " << parent_distance_T
                << ", parent_distance_V = " << parent_distance_V << std::endl
                << std::endl;
    }
  }
  else
  {
    ...
    int min_distance_T = T_matrix_can(parent_node_can, observed_child_nodes[0]);
    int min_distance_V = V_matrix_can(parent_node_can, observed_child_nodes[0]);
    double min_time_T = sample_times[observed_child_nodes[0]];
    double min_time_V = sample_times[observed_child_nodes[0]];
    
    for(size_t i = 1; i < observed_child_nodes.size(); i++)
    {
      int current_child_node = observed_child_nodes[i];
      
      int current_distance_T = T_matrix_can(parent_node_can, current_child_node);
      int current_distance_V = V_matrix_can(parent_node_can, current_child_node);
      if(current_distance_T <= min_distance_T)
      {
        min_distance_T = current_distance_T;
    ...
    int triangle_min_T = ReturnMinDistanceTriangle(imp_idx, T_matrix_can);
    min_distance_T = std::min(min_distance_T, triangle_min_T);
    
    int triangle_min_V = ReturnMinDistanceTriangle(imp_idx, V_matrix_can);
    min_distance_V = std::min(min_distance_V, triangle_min_V);
    ...
    double lower_T = min_time_T - sample_times[parent_node_can];
    double time_ratio_T = upper/lower_T;
    
    double lower_V = min_time_V - sample_times[parent_node_can];
    double time_ratio_V = upper/lower_V;
    
    if(time_ratio_T < 0 || time_ratio_T > 1)
    {
      ...
      std::cout << std::endl << "Error with T distances when imputing interior nodes" << std::endl;
      ...
      //std::cout << ""
      std::cout << "min time = " << min_time_T << std::endl;
      std::cout << "Time ratio = " << time_ratio_T << std::endl;
    ...
    if(time_ratio_T < 0 || time_ratio_V > 1)
    {
      //IntegerVector gen_source_cur = ReturnGenSourceVector_Rcpp(data_cur);
      //IntegerVector gen_source_can = ReturnGenSourceVector_Rcpp((*this));
      //Rcout << gen_source_cur << std::endl;
      //Rcout << gen_source_can << std::endl;
      std::cout << std::endl << "Error with T distances" << std::endl;
      std::cout << "parent node = " << parent_node_can << ", observed children = ";
      PrintVector(observed_child_nodes);
      std::cout << "sample_times[imputed_idx_can] = " << sample_times[imp_idx] << std::endl;
      std::cout << "sample_times[parent_node] = " << sample_times[parent_node_can] << std::endl;
    ...
    if(verbose)
    {
      std::cout << "  Different config" << ", min_distance_T = "
                ...
                << ", parent_distance = "
                << parent_distance_V << ", time_ratio_T = "
                << time_ratio_T
                << ", time_ratio_V = " << time_ratio_V << ", contribution = "
                << contribution
                << std::endl << std::endl;
    }
  }
  ...
  T_matrix_can(parent_node_can, imp_idx) = parent_distance_T;
  T_matrix_can(imp_idx, parent_node_can) = parent_distance_T;
  V_matrix_can(parent_node_can, imp_idx) = parent_distance_V;
  V_matrix_can(imp_idx, parent_node_can) = parent_distance_V;
  for(size_t i = 0; i < observed_child_nodes.size(); i++)
  {
    int current_child_node = observed_child_nodes[i];
    T_matrix_can(imp_idx, current_child_node) = T_matrix_can(parent_node_can, current_child_node) -
      T_matrix_can(parent_node_can, imp_idx);
    T_matrix_can(current_child_node, imp_idx) = T_matrix_can(parent_node_can, current_child_node) -
      T_matrix_can(parent_node_can, imp_idx);
    
    V_matrix_can(imp_idx, current_child_node) = V_matrix_can(parent_node_can, current_child_node) -
      V_matrix_can(parent_node_can, imp_idx);
    V_matrix_can(current_child_node, imp_idx) = V_matrix_can(parent_node_can, current_child_node) -
      V_matrix_can(parent_node_can, imp_idx);
  }
...
void Data::UpdateInfectionTime(int &nacc, bool verbose)
{
  ...
  if(target_source != -1)
  {
    ...
    if(log(U) < data_can.loglik - loglik + log_prop_ratio)
    {
      ...
      T_matrix = data_can.T_matrix;
      V_matrix = data_can.V_matrix;
...
// [[Rcpp::export]]
void MCMC_SEIR_K80(List MCMC_options,
                  int sequence_length,
                  NumericVector t_e,
                  NumericVector t_i, 
                  NumericVector t_r,
                  ...
                  IntegerMatrix V_matrix)
{
  ...
  int nacc_transition_rate = 0;
  int nacc_transition_rate_prop = 0;
  int nacc_transversion_rate = 0;
  int nacc_transversion_rate_prop = 0;
  int nacc_exp = 0;
  int nacc_exp_prop = 0;
  int nacc_inf = 0;
  int nacc_inf_prop = 0;
  ...
  for(int iter = 1; iter < max_iterations; iter++)
  {
    ...
    // Update transition_rate by MH
    int transition_rate_flag = debug_flags["transition_rate"];
    if(transition_rate_flag == 0)
    {
      double prop_var = proposal_variance["transition_rate"];
      double transition_rate_lower = prior_parameters["transition_rate_lower"];
      double transition_rate_higher = prior_parameters["transition_rate_higher"];
      nacc_transition_rate_prop++;
      data.UpdateParameterMetropolisHastingsUniform(6, 
                                                    prop_var,
                                                    transition_rate_lower,
                                                    transition_rate_higher,
                                                    nacc_transition_rate);
    }
    
    // Update transversion_rate by MH
    int transversion_rate_flag = debug_flags["transversion_rate"];
    if(transversion_rate_flag == 0)
    {
      double prop_var = proposal_variance["transversion_rate"];
      double transversion_rate_lower = prior_parameters["transversion_rate_lower"];
      double transversion_rate_higher = prior_parameters["transversion_rate_higher"];
      nacc_transversion_rate_prop++;
      data.UpdateParameterMetropolisHastingsUniform(7, 
                                                    prop_var,
                                                    transversion_rate_lower,
                                                    transversion_rate_higher,
                                                    nacc_transversion_rate);
    }
  ...
  double transition_prob = (double)nacc_transition_rate/(double)nacc_transition_rate_prop;
  double transversion_prob = (double)nacc_transversion_rate/(double)nacc_transversion_rate_prop;
  ...
  std::cout << "Acceptance probabilities: kappa = " << kappa_prob
            << ", delta = " << delta_prob
            << ", zeta = " << zeta_prob
            << ", transition_rate = " << transition_prob
            << ", transversion_rate = " << transversion_prob
            << ", exp_time = " << exp_prob
            << ", inf_time = " << inf_prob
            << ", dist_prop = " << dist_prob
            << std::endl;
